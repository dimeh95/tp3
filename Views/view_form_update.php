...
<form action = "?controller=set&action=update" method="post">
    <p> <input type="hidden" name="id" value="<?= e($id) ?>" /> <label> Name: <input type="text" name="name" value="<?= e($name) ?>"/> </label> 
    ...
    <p> <label> County: <input type="text" name="county" value="<?= e($county) ?>"/></label> </p>

    <p>
    <?php foreach ($categories as $v) : ?>
        <label> <input type="radio" name="category" value="<?= e($v) ?>" <?php if ($v == $category) {
